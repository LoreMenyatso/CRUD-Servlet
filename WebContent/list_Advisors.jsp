...
<head>
  ...
  <blockquote class="blockquote">
    <p3>A directory of all the registered business advisors.</p3>
    <footer class="blockquote-footer">2020</footer>
			...
			<!-- buttons that search and add  -->
					...
					<div class="col">
   	    				<input type="button" value="Add Advisor" onclick="window.location.href='add-advisor-form.jsp'; return false;"
   		...
   		<br>
               ...
               <table class="table table-hover">
                ...
                <tr>
                     ...
                     <c:forEach var="tempAdvisor" items="${ADVISOR_LIST}">         
                      
                         <c:url var="tempLink" value="ControllerServlet">
                         <c:param name="command" value="loadAdvisors"/>
                         <c:param name="AdvisorId" value="${tempAdvisor.id}"/>
                         </c:url>
                         
                         <c:url var="deletLink" value="ControllerServlet">
                         <c:param name="command" value="deleteAdvisors"/>
                         <c:param name="AdvisorId" value="${tempAdvisor.id}"/>
                         </c:url>
                         
                         
                         <tr>
                                <td>${tempAdvisor.first_name}</td>
                                <td>${tempAdvisor.last_name}</td>
                                <td>${tempAdvisor.email}</td>
